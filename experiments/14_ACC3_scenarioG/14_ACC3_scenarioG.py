...
dna_file_name = '14_ACC3_scenarioG'                     # Name of the DNA file (without .dna!)
dna_path = os.path.join(dir_path, dna_file_name)       # Path of the DNA file

airsim_settings_file_name = '14_ACC3_scenarioG.json'    # Name of the AirSim settings .json
...
for j in range(simulations):
    ...
    for i in range(1, number_of_processors+1):
        run_command_gpp = f"""start "GPP {i}" "{dir_path}\\14_GeneralPurposeProcessor.exe" /DNA:{dna_path} /L:{log_level} {i} """
    ...
    os.system("taskkill /im 14_GeneralPurposeProcessor.exe")
