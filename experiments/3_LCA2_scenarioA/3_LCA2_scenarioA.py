...
This script runs scenario A of LCA2 100 times.
...
dna_file_name = '3_LCA2_scenarioA'                     # Name of the DNA file (without .dna!)
dna_path = os.path.join(dir_path, dna_file_name)       # Path of the DNA file

airsim_settings_file_name = '3_LCA2_scenarioA.json'    # Name of the AirSim settings .json
...
for j in range(simulations):
    ...
    for i in range(1, number_of_processors+1):
        run_command_gpp = f"""start "GPP {i}" "{dir_path}\\3_GeneralPurposeProcessor.exe" /DNA:{dna_path} /L:{log_level} {i} """
        os.system(run_command_gpp)
    time.sleep(5)

    # Run ADNAAirSimClient.exe
    run_command_client = f"""start "ADNAAirSimClient" "{dir_path}\\..\\..\\Client\\build\\x64\\Debug\\ADNAAirSimClient.exe" """
    ...
    os.system("taskkill /im 3_GeneralPurposeProcessor.exe")
