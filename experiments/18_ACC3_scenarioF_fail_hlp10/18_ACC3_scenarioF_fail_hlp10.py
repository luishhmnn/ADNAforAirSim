...
This script runs `scenario F - fail` of ACC3 100 times.
...
timed_shutdown_fail = 8000                             # fail after x ms
timed_shutdown = 13000                                 # GeneralPurposeProcessor shutdown after x ms

# Paths & file names
dir_path = pathlib.Path(__file__).parent.resolve()     # Directory path of this script
cwd = os.getcwd()                                      # current working directory (pwd)

dna_file_name = '18_ACC3_scenarioF_fail_hlp10'                     # Name of the DNA file (without .dna!)
dna_path = os.path.join(dir_path, dna_file_name)       # Path of the DNA file

airsim_settings_file_name = '18_ACC3_scenarioF_fail_hlp10.json'    # Name of the AirSim settings .json
...
for j in range(simulations):
    ...
    run_command_gpp = f"""start "GPP 1" "{dir_path}\\18_GeneralPurposeProcessor.exe" /DNA:{dna_path} /H:10 /K:{timed_shutdown_fail} /L:{log_level} 1 """
    os.system(run_command_gpp)
    for i in range(2, number_of_processors+1):
        run_command_gpp = f"""start "GPP {i}" "{dir_path}\\18_GeneralPurposeProcessor.exe" /DNA:{dna_path} /H:10 /K:{timed_shutdown} /L:{log_level} {i} """
        os.system(run_command_gpp)
    time.sleep(5)
    ...
    # 5. Wait until target speed is reached ---------------------------------------------------------------------------
    time.sleep(10)
    ...
    os.system("taskkill /f /im 18_GeneralPurposeProcessor.exe")
