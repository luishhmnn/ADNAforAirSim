...
This script runs scenario B of LCA1 100 times.
...
dna_file_name = '2_LCA1_scenarioB'                     # Name of the DNA file (without .dna!)
dna_path = os.path.join(dir_path, dna_file_name)       # Path of the DNA file

airsim_settings_file_name = '2_LCA1_scenarioB.json'    # Name of the AirSim settings .json
...
for j in range(simulations):
    ...
    for i in range(1, number_of_processors+1):
        run_command_gpp = f"""start "GPP {i}" "{dir_path}\\2_GeneralPurposeProcessor.exe" /DNA:{dna_path} /L:{log_level} {i} """
    ...
    os.system("taskkill /im 2_GeneralPurposeProcessor.exe")
