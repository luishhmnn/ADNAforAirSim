...
dna_file_name = '17_ACC3_scenarioF_fail'                     # Name of the DNA file (without .dna!)
dna_path = os.path.join(dir_path, dna_file_name)       # Path of the DNA file

airsim_settings_file_name = '17_ACC3_scenarioF_fail.json'    # Name of the AirSim settings .json
...
for j in range(simulations):
    ...
    run_command_gpp = f"""start "GPP 1" "{dir_path}\\17_GeneralPurposeProcessor.exe" /DNA:{dna_path} /K:{timed_shutdown_fail} /L:{log_level} 1 """
    os.system(run_command_gpp)
    for i in range(2, number_of_processors+1):
        run_command_gpp = f"""start "GPP {i}" "{dir_path}\\17_GeneralPurposeProcessor.exe" /DNA:{dna_path} /K:{timed_shutdown} /L:{log_level} {i} """
        os.system(run_command_gpp)
    time.sleep(5)
