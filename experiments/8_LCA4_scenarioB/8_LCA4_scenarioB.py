...
# 1. Settings --------------------------------------------------------------------------------
simulations = 130                                      # Number of simulations
...
dna_file_name = '8_LCA4_scenarioB'                     # Name of the DNA file (without .dna!)
dna_path = os.path.join(dir_path, dna_file_name)       # Path of the DNA file

airsim_settings_file_name = '8_LCA4_scenarioB.json'    # Name of the AirSim settings .json
...
for j in range(simulations):
    ...
    for i in range(1, number_of_processors+1):
        run_command_gpp = f"""start "GPP {i}" "{dir_path}\\8_GeneralPurposeProcessor.exe" /DNA:{dna_path} /L:{log_level} {i} """
    ...
    # NOTE: Establish connection after starting the LCA, because its startup time is very long
    # 4. Establish connection and start the simulation -------------------------------------------
    # open `Control` menu and select `Connect to AirSim`
    pyautogui.hotkey('alt', 'C')
    pyautogui.press('C')
    ...
    os.system("taskkill /im 8_GeneralPurposeProcessor.exe")
